...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::GTC);
    ...
    if (!settings.contains("nAnonymizeGastroCoinAmount"))
        settings.setValue("nAnonymizeGastroCoinAmount", 1000);

    nAnonymizeGastroCoinAmount = settings.value("nAnonymizeGastroCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGastroCoinAmount"))
        SoftSetArg("-anonymizegastrocoinamount", settings.value("nAnonymizeGastroCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in gastrocoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGastroCoinAmount:
            return QVariant(nAnonymizeGastroCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGastroCoinAmount:
            nAnonymizeGastroCoinAmount = value.toInt();
            settings.setValue("nAnonymizeGastroCoinAmount", nAnonymizeGastroCoinAmount);
            emit anonymizeGastroCoinAmountChanged(nAnonymizeGastroCoinAmount);
            break;
        case CoinControlFeatures:
