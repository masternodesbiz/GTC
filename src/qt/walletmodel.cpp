...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered gastrocoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 GTC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal gastrocoin:URI (gastrocoin:XyZ...?message=example)
            {
...
static void NotifyzgtcReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzgtcReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzgtcReset.connect(boost::bind(NotifyzgtcReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzgtcReset.disconnect(boost::bind(NotifyzgtcReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zgtcTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
