...
#include "ui_zgtccontroldialog.h"
...
std::set<std::string> ZGTCControlDialog::setSelectedMints;
std::set<CMintMeta> ZGTCControlDialog::setMints;

ZGTCControlDialog::ZGTCControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZGTCControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZGTCControlDialog::~ZGTCControlDialog()
{
    delete ui;
}

void ZGTCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGTCControlDialog::updateList()
{
...
void ZGTCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGTCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZGTC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGTCControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZGTCControlDialog::GetSelectedMints()
{
...
void ZGTCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
