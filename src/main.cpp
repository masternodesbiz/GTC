...
#if defined(NDEBUG)
#error "GastroCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uGTC) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
		return error("%s : zgtc spend with serial %s is already in block %d\n", __func__,
...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	//Check to see if the zgtc is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		if (!spend.HasValidSignature())
			return error("%s: V2 zgtc spend does not have a valid signature", __func__);

		libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
		if (tx.IsCoinStake())
			expectedType = libzerocoin::SpendType::STAKE;
		if (spend.getSpendType() != expectedType) {
			return error("%s: trying to spend zgtc without the correct spend type. txid=%s", __func__,
				tx.GetHash().GetHex());
		}
	}
	...
	if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
		...
		return error("%s : zgtc spend with serial %s from tx %s is not in valid range\n", __func__,
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zgtc spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
					return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
						tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zgtc");
			}
		}
		else {
			LOCK(pool.cs);
			...
			// Check that zgtc mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 160000 && nHeight > 120000) {
		nSubsidy = 50 * COIN;
	}
	else if (nHeight <= 185000 && nHeight > 160000) {
		nSubsidy = 60 * COIN;
	}
	else if (nHeight <= 300000 && nHeight > 185000) {
		nSubsidy = 65 * COIN;
	}
	else if (nHeight <= 600000 && nHeight > 300000) {
		nSubsidy = 75 * COIN;
	}
	else if (nHeight <= 900000 && nHeight > 600000) {
		nSubsidy = 80 * COIN;
	}
	else if (nHeight <= 1200000 && nHeight > 900000) {
		nSubsidy = 85 * COIN;
	}
	else if (nHeight <= 1500000 && nHeight > 1200000) {
		nSubsidy = 90 * COIN;
	}
	else if (nHeight <= 1900000 && nHeight > 1500000) {
		nSubsidy = 90 * COIN;
	}
	else if (nHeight <= 2300000 && nHeight > 1900000) {
		nSubsidy = 95 * COIN;
	}
	else if (nHeight <= 2500000 && nHeight > 2300000) {
		nSubsidy = 96 * COIN;
	}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZGTCStake)
{
    int64_t ret = 0;
...
int64_t GetMasternodePayment (int nHeight, unsigned int mnLevel, int64_t blockValue, int nMasternodeCount, bool isZGTCStake) {
  return GetMasternodePayment (nHeight, blockValue, nMasternodeCount, isZGTCStake) * Params ().getMasternodePhaseWeight (mnLevel, nHeight) / Params ().getMasternodePhaseWeight (0, nHeight);
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from GastroCoin
...
{
	RenameThread("gastrocoin-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZGTCMinted()
{
...
void RecalculateZGTCSpent()
{
	...
	while (true) {
		...
		//Remove spends from zgtc supply
...
bool RecalculateGTCSupply(int nHeightStart)
{
...
{
	// GastroCoin: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZGTCSupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
		RecalculateZGTCMinted();
		RecalculateZGTCSpent();
		RecalculateGTCSupply(Params().Zerocoin_StartHeight());
	}

	//Track zgtc money supply in the block index
	if (!UpdateZGTCSupply(block, pindex))
		return state.DoS(100, error("%s: Failed to calculate new zgtc supply for block=%s height=%d", __func__,
	...
	//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zGTCSpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent zgtc spends in this block
		if (tx.IsZerocoinSpend()) {
			for (const CTxIn& txIn : tx.vin) {
				if (txIn.scriptSig.IsZerocoinSpend()) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zgtc serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	if (nHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zgtc stake block is less than allowed start height", __func__);

	if (CZGTCStake* zgtc = dynamic_cast<CZGTCStake*>(stake)) {
		CBlockIndex* pindexFrom = zgtc->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zgtc stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zgtc stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zgtc->GetValue()));
		if (nChecksum200 != zgtc->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zgtc->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZGTC() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked zgtc fails context checks", __func__));
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> GTCInputs;
		std::vector<CTxIn> zgtcInputs;

		for (CTxIn stakeIn : stakeTxIn.vin) {
			if (stakeIn.scriptSig.IsZerocoinSpend()) {
				zgtcInputs.push_back(stakeIn);
			}
			else {
				GTCInputs.push_back(stakeIn);
			}
		}
		const bool hasGTCInputs = !GTCInputs.empty();
		const bool hasZGTCInputs = !zgtcInputs.empty();
		...
		for (CTransaction tx : block.vtx) {
			for (CTxIn in : tx.vin) {
				...
				if (hasGTCInputs)
					...
					for (CTxIn GTCIn : GTCInputs) {
						if (GTCIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			// Now that this loop if completed. Check if we have zgtc inputs.
			if (hasZGTCInputs) {
				for (CTxIn zGTCInput : zgtcInputs) {
					CoinSpend spend = TxInToZerocoinSpend(zGTCInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zgtc");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zgtc mints and %d zgtc spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// GastroCoin: We use certain sporks during IBD, so check to see if they are
...
int ActiveProtocol()
{
	// SPORK_14 was used for 90003 (v1.0.0+)
	     if (IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
	        return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

	// SPORK_15 is used for 90003 (v1.0.1+)
