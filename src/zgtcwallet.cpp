...
CzgtcWallet::CzgtcWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zgtc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGTCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGTCSeed_deprecated()) {
                    LogPrintf("%s: Updated zgtc seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zgtc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zgtc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzgtcWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGTCCount(nCountLastUsed);
    else if (!walletdb.ReadZGTCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzgtcWallet::Lock()
{
    seedMaster = 0;
}

void CzgtcWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzgtcWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGTC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzgtcWallet::LoadMintPoolFromDB()
{
...
void CzgtcWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzgtcWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzgtcWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zgtcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzgtcWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGTC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zgtcTracker which also adds to database
    pwalletMain->zgtcTracker->Add(dMint, true);
...
void CzgtcWallet::SeedToZGTC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzgtcWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzgtcWallet::GenerateDeterministicZGTC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzgtcWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzgtcWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
