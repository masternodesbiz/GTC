...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between GTC and zGTC
    for(const CTxIn& txin : vin) {
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // GTC: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
