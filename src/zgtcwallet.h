...
#ifndef GastroCoin_ZGTCWALLET_H
#define GastroCoin_ZGTCWALLET_H
...
public:
    CzgtcWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZGTC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZGTC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //GastroCoin_ZGTCWALLET_H
