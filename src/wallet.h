...
enum AvailableCoinsType {
    ...
    EXCEPT_DENOMINATED_OR_MASTERNODE_COLLATERAL = 4, // ONLY_NONDENOMINATED and not 15000 GTC at the same time
    ONLY_MASTERNODE_COLLATERAL = 5,       // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zgtc send
enum ZerocoinSpendStatus {
    ZGTC_SPEND_OKAY = 0,                            // No error
    ZGTC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZGTC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZGTC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZGTC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZGTC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZGTC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZGTC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZGTC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZGTC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZGTC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZGTC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZGTC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZGTC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZGTC_SPENT_USED_ZGTC = 14,                      // Coin has already been spend
    ZGTC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZGTC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZGTCBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZGTCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzgtcAuto) const;
    ...
    */
    ...
    CzgtcWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzgtcTracker> zgtcTracker;
    ...
    void setZWallet(CzgtcWallet* zwallet)
    {
        zwalletMain = zwallet;
        zgtcTracker = std::unique_ptr<CzgtcTracker>(new CzgtcTracker(strWalletFile));
    }

    CzgtcWallet* getZWallet() { return zwalletMain; }
    ...
    /** zgtc reset */
    boost::signals2::signal<void()> NotifyzgtcReset;
