...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZGTCStake(spend));
    } else {
        ...
        CGTCStake* GTCInput = new CGTCStake();
        GTCInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(GTCInput);
    }
