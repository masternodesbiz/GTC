...
public:
    ...
    bool WriteZGTCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGTCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGTCSeed_deprecated(uint256& seed);
    bool EraseZGTCSeed();
    bool EraseZGTCSeed_deprecated();

    bool WriteZGTCCount(const uint32_t& nCount);
    bool ReadZGTCCount(uint32_t& nCount);
