...
CzgtcTracker::CzgtcTracker(std::string strWalletFile)
{
...
CzgtcTracker::~CzgtcTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzgtcTracker::Init()
{
...
bool CzgtcTracker::Archive(CMintMeta& meta)
{
...
bool CzgtcTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzgtcTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzgtcTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzgtcTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzgtcTracker::GetSerialHashes()
{
...
CAmount CzgtcTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzgtcTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzgtcTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzgtcTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzgtcTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzgtcTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzgtcTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzgtcTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzgtcTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzgtcTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzgtcTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzgtcTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzgtcTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzgtcTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzgtcTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzgtcTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzgtcTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzgtc from DB\n", __func__, listDeterministicDB.size());
    }
