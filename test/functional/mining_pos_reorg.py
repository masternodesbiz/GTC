...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected GTC and zGTC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zGTCsupply']:
                assert_equal(node['zGTCsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check GTC and zGTC supply at the beginning
        # ------------------------------------------
        # zGTC supply: 2 coins for each denomination
        ...
        # GTC supply: block rewards minus burned fees for minting
        ...
        # Verify that GTC and zGTC supplies were properly updated after the spends and reorgs
        self.log.info("Check GTC and zGTC supply...")
